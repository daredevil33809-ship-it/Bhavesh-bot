...
import functools # Used for running blocking functions in an executor
...
# --- Blocking I/O Functions (Now handled by asyncio.to_thread) ---
...
# --- ASYNC Wrappers (for running I/O inside aiogram event loop) ---

# This ensures all blocking I/O calls run in a separate thread, fixing the "Timeout context manager" error
...
async def cmd_start(message: Message):
    if message.from_user:
        # Note: add_user is synchronous, but runs fast on a new user.
        # For safety, we wrap blocking calls, but add_user is kept sync here for simplicity
        # as it usually involves minimal file writing.
        pass # User addition logic here (kept outside the handler for simplicity)
    
    # We load users here if needed, but keeping it simple for now
...
async def cmd_refresh(message: Message):
    ...
    # FIX: Using the async wrapper for blocking file I/O
    await load_movies()
        ...
        if message.chat.id == LIBRARY_CHANNEL_ID or (message.chat.username and message.chat.username.lower() == LIBRARY_CHANNEL_USERNAME.lower().replace('@', '')):
            if message.document or message.video:
                ...
                if title and title != "Unknown Movie" and file_id:
                    # FIX: Using the async wrapper for blocking file I/O
                    if await add_movie(title, file_id):
...
async def telegram_webhook():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            # FINAL FIX: Correct signature for aiogram v3 (bot, update)
...
def initialize_app():
    ...
    # We run blocking I/O initialization outside the main thread, 
    # but since this runs before Gunicorn starts the workers, we can keep it sync here.
