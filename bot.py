...
# --- ZAROORI CHANGE: check_user_membership FUNCTION HATA DIYA GAYA HAI ---
# --- Ab access sirf button press karne par milega. ---
...
You have full access to all bot features."""
    ...
    # --- CHANGE: Agar user verified nahi hai, toh join button dikhao ---
    ...
    # Agar user pehle se hi verified hai (button daba chuka hai), toh seedha welcome message
    else:
...
async def process_joined(callback: types.CallbackQuery):
    # --- ZAROORI CHANGE: Button dabate hi user ko verified_users set mein add karo ---
    if callback.from_user:
        # User ko verified set mein joda, ab unhe access mil gaya
...
अभी सब कुछ फ्री है। बस फ़िल्म का नाम टाइप करें और एंजॉय करें!"""
    ...
    if callback.message and isinstance(callback.message, Message):
        # Keyboard ko hata kar welcome message dikhao
        await callback.message.edit_text(welcome_text, reply_markup=None) 
    # Confirmation message ko 'Access granted' mein badla
...
async def handle_search(message: Message):
    try:
        ...
        # --- ZAROORI CHANGE: Simplified Access Check ---
        # Ab sirf verified_users set mein check hoga
...
async def send_movie(callback: types.CallbackQuery):
    try:
        ...
        # Security check: Ensure non-admin user is verified
