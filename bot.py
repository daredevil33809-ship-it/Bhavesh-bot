...
async def process_joined(callback: types.CallbackQuery):
    # NOTE: Membership is now checked in handle_search for strict access control.
...
अभी सब कुछ फ्री है। बस फ़िल्म का नाम टाइप करें और एंजॉय करें!"""
    ...
    await callback.answer("Membership check required before searching.")
...
async def handle_search(message: Message):
    try:
        ...
        # Access Control Check: Agar user admin nahi hai aur membership check fail ho
        if user_id not in ADMIN_IDS and not await check_user_membership(user_id):
            keyboard = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text=f"🔗 Join Channel", url=f"https://t.me/{JOIN_CHANNEL_USERNAME.replace('@', '')}")],
                [InlineKeyboardButton(text=f"👥 Join Group", url=f"https://t.me/{JOIN_GROUP_USERNAME.replace('@', '')}")],
                [InlineKeyboardButton(text="✅ I Joined", callback_data="joined")]
            ...
            await message.answer(
                "🛑 **Access Denied!** Please join our channel and group to use the search function.",
                reply_markup=keyboard,
