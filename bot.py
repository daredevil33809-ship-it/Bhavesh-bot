...
# check_user_membership function hata diya gaya hai
...
You have full access to all bot features."""
    ...
    elif message.from_user and message.from_user.id not in verified_users:
        # Jab user verified nahi hai, tab hi join button dikhao
        ...
        await message.answer(
            "Welcome! Please click 'I Joined' button below to continue:",
            reply_markup=keyboard
        )
    
    else:
        # Agar user verified hai, toh seedha welcome message
...
async def process_joined(callback: types.CallbackQuery):
    if callback.from_user:
        # Button dabate hi user ko verified set mein add karo
...
अभी सब कुछ फ्री है। बस फ़िल्म का नाम टाइप करें और एंजॉय करें!"""
    ...
    if callback.message and isinstance(callback.message, Message):
        # Keyboard hatao (reply_markup=None) aur message edit karo
        await callback.message.edit_text(welcome_text, reply_markup=None) 
    await callback.answer("✅ Access granted! You can now start searching for movies.")
...
async def handle_search(message: Message):
    try:
        ...
        if user_id not in ADMIN_IDS and user_id not in verified_users:
            ...
            await message.answer(
                "🛑 **Access Denied!** Please click 'I Joined' button (or run /start) to use the search function.",
                reply_markup=keyboard,
...
async def send_movie(callback: types.CallbackQuery):
    try:
        ...
        if callback.from_user.id not in ADMIN_IDS and callback.from_user.id not in verified_users:
             await callback.answer("🛑 Access Denied. Please press the 'I Joined' button first.")
             return
...
async def delete_and_start_polling():
    print("Deleting old Telegram Webhook...")
    await bot.delete_webhook(drop_pending_updates=True) 
    print("Webhook deleted successfully.")
    
    # 5 second ki deri taaki pichla process band ho jaaye
    await asyncio.sleep(5) 
    
    print("Starting Long Polling...")
    # Polling ko ek loop mein daalte hain taaki agar ConflictError aaye to dobara Webhook delete karke shuru ho sake
    while True:
        try:
            await dp.start_polling(bot)
            break  # Agar successfully start ho gaya, toh loop se bahar nikal jao
        except Exception as e:
            error_msg = str(e)
            if "Conflict" in error_msg:
                print(f"Polling Conflict Detected: Retrying setup. Error: {error_msg}")
                # Conflict hone par dobara webhook delete karo aur thoda aur ruko
                await bot.delete_webhook(drop_pending_updates=True)
                await asyncio.sleep(10) # 10 second ka lamba intezaar
            else:
                print(f"FATAL ERROR: Bot could not start polling. {e}")
                # Agar koi aur serious error ho toh phir bhi loop mein rehna theek hai, lekin
                # yahan hum bas conflict ko handle kar rahe hain.
...
if __name__ == "__main__":
    ...
    try:
        asyncio.run(delete_and_start_polling())
    except Exception as e:
