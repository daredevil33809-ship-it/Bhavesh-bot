...
from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton, Message, Update
...
if not BOT_TOKEN:
    # If the token is not set, raise an error immediately upon startup
...
# FIX: Use DefaultBotProperties for new aiogram versions
bot = Bot(
    token=BOT_TOKEN, 
...
# --- Blocking I/O Functions (Synchronous Versions for safety) ---

def _build_movies_index():
    global movies_index
    movies_index = {}
...
def _add_movie(title: str, file_id: str) -> bool:
...
# --- ASYNC Wrappers (for safety when running I/O inside async code) ---
async def load_movies():
    await asyncio.to_thread(_load_movies)

async def save_movies():
    await asyncio.to_thread(_save_movies)

async def load_users():
    await asyncio.to_thread(_load_users)

async def save_users():
    await asyncio.to_thread(_save_users)

async def add_movie(title: str, file_id: str) -> bool:
    return await asyncio.to_thread(_add_movie, title, file_id)

# --- Other Utility Functions (keeping original logic) ---
...
async def cmd_start(message: Message):
    if message.from_user:
        await asyncio.to_thread(add_user, user_id=message.from_user.id, username=message.from_user.username, first_name=message.from_user.first_name)
...
async def cmd_refresh(message: Message):
    ...
    # Reloading the index from disk is a blocking operation, so run it in a thread
    await asyncio.to_thread(_load_movies)
    search_cache.clear()
        ...
        if message.chat.id == LIBRARY_CHANNEL_ID or (message.chat.username and message.chat.username.lower() == LIBRARY_CHANNEL_USERNAME.lower().replace('@', '')):
            if message.document or message.video:
                ...
                if title and title != "Unknown Movie" and file_id:
                    # We use the sync function here to ensure data consistency
                    if await asyncio.to_thread(_add_movie, title, file_id):
...
async def handle_search(message: Message):
    try:
        ...
        # Fuzzy search is a CPU-bound operation, running it sync is safe here
        results = await asyncio.to_thread(advanced_fuzzy_search, query, limit=15)
...
async def telegram_webhook():
    ...
    if request.method == "POST":
        ...
        try:
            # Final FIX: Correct signature for aiogram v3
            ...
            # Final Stability FIX: Manually close the session to prevent gevent/loop conflict 
...
def initialize_app():
    ...
    # We call the synchronous versions of load functions here.
    _load_movies() 
    _load_users()
